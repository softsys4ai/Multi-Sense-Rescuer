...
class PPOTrainer(BaseRLTrainer):
    ...
    def _eval_checkpoint(
        ...
        log_str = ""
        if config.EVAL.SPLIT.startswith('test'):
            log_str += "TEST: "
        else:
            log_str += "EVAL: "
	
        log_str += f"Checkpoint: {checkpoint_index} \n"
        log_str += f"Average episode reward: {episode_reward_mean:.6f} \n"
        for metric_uuid in self.metric_uuids:
            log_str += f"Average episode {metric_uuid}: {episode_metrics_mean[metric_uuid]:.6f} \n"                                            
        logger.info(log_str)
