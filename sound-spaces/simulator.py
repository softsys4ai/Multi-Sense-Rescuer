...
class Simulator(SimulatorBackend):
    ...
    def get_sensor_observations_async_finish(
        ...
        for agent_id in agent_ids:
            ...
            for sensor_uuid, sensor in self.__sensors[agent_id].items():
                agent_observations[sensor_uuid] = sensor._get_observation_async(sensor_uuid)
    ...
    def get_sensor_observations(
        ...
        for agent_id in agent_ids:
            ...
            for sensor_uuid, sensor in self.__sensors[agent_id].items():
                agent_observations[sensor_uuid] = sensor.get_observation(sensor_uuid)
            observations[agent_id] = agent_observations
        if return_single:
            return next(iter(observations.values()))
        return observations
...
class Sensor:
    ...
    """#KS
    Args:
    audio_sensor_uuid: if sensor is audio sensor, then pass which sensor observation to get.
                    Needed for multiple sound sources
    """
    def get_observation(self, audio_sensor_uuid=None) -> Union[ndarray, "Tensor"]:
        if self._spec.sensor_type == SensorType.AUDIO:
            return self._get_audio_observation(audio_sensor_uuid)
    ...
    def _get_observation_async(self, audio_sensor_uuid=None) -> Union[ndarray, "Tensor"]:
        if self._spec.sensor_type == SensorType.AUDIO:
            return self._get_audio_observation(audio_sensor_uuid)
    ...
    def _get_audio_observation(self, audio_sensor_uuid) -> Union[ndarray, "Tensor"]:
        assert self._spec.sensor_type == SensorType.AUDIO

        audio_sensor = self._agent._sensors[audio_sensor_uuid]
