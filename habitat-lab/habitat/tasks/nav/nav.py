...
class Success(Measure):
    ...
    def update_metric(
        ...
        ## ].get_metric()
        ## print("SUCCESS update_metric called")
        if (##########KS
            ## hasattr(task, "is_stop_called")
            ## and task.is_stop_called  # type: ignore
            ## and 
            ## distance_to_target < self._config.SUCCESS_DISTANCE
            self._sim._episode_success()
        ):
            self._metric = 1.0
        else:
            self._metric = 0.0
...
class DistanceToGoal(Measure):
    ...
    def update_metric(
        ...
        # print("______________update dist2goal metric called______________")
        current_position = self._sim.get_agent_state().position
        ...
        if self._previous_position is None or not np.allclose(
            ...
            if self._config.DISTANCE_TO == "POINT":
                distance_to_target = self._sim._distance_target() ##KS
